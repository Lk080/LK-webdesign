...
const sites = { lk: { title: 'LK Webdesign', source: 'docs', frozen: false }, kelmora: { title: 'Kelmora', source: 'docs/demos/vakman', frozen: true } };
...
function parse(argv) {
  ...
  if (!sites[options.site]) throw Error('Kies expliciet --site lk of --site kelmora. Nieuwe sites pas registreren zodra ze bestaan.');
...
function plan(options) {
  ...
  const jobs = selected.map(check => {
    ...
    if (['smoke', 'functional', 'axe', 'visual'].includes(check)) {
      ...
      if (!visual) args.push(check === 'axe' ? 'accessibility.spec.cjs' : 'sites.spec.cjs', ...(check === 'functional' && options.site === 'kelmora' ? ['kelmora-flows.spec.cjs'] : []));
      args.push('--grep', grep);
