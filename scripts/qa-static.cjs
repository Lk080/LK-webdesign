...
async function main() {
  ...
  const ctx = context(site, 'static');
  const output = reserve(ctx, mode === 'all' ? 'static' : mode);
  console.log(`QA run: ${ctx.dir}`);
  const project = getProject(site);
  const website = () => project.name;
  const inScope = file => site !== 'lk' || !file.includes('/docs/demos/');
  await fs.mkdir(output, { recursive: true });
  const files = (await walk(path.join(root, project.source))).filter(f => inScope(f) && !/\.min\.(js|css)$/.test(f));
  let errors = 0;
  for (const tool of mode === 'all' ? ['html', 'css', 'js', 'links'] : [mode]) {
    ...
    if (tool === 'links') {
      ...
      try {
        ...
        for (const target of [{ id: site, name: project.name, path: project.route }]) {
          ...
          const result = await new LinkChecker().check({ path: origin + target.path, recurse: true, checkCss: true, checkFragments: true, redirects: 'error', retry: false, retryErrors: false, timeout: 10000, concurrency: 4,
            linksToSkip: async link => { try { const u = new URL(link); return u.origin !== origin || (site === 'lk' ? u.pathname.startsWith('/demos/') : !u.pathname.startsWith(project.route)); } catch { return true; } },
          });
    ...
    await fs.writeFile(path.join(output, `${tool}.json`), JSON.stringify(stamp(ctx, tool, report), null, 2));
